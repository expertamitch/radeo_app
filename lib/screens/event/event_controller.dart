...
class EventController extends GetxController{
 ...
 Future<EventDetailModel?> addInvitee(Map<String, dynamic> map, String id) async {
  try {
   showLoader();

   var data = await BackendRepo().addInvitee(
    data: map,id: id
   );
   if (data?.message != null) {
    showSuccessSnackBar(data?.message??'');
 ...
 executeSearch(String searchedText) {
  tempEventList.value = eventlist.map((e) => EventInfoModel.clone(e)).toList();
